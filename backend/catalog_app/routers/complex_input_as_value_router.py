...
@complex_input_as_value_router.get(
    "/", response_model=List[schemas.ComplexInputAsValue]
)
def read_list(
    skip: int = 0,
    limit: int = 100,
...
@complex_input_as_value_router.get(
    "/{complex_input_as_value_id}", response_model=schemas.ComplexInputAsValue
)
def read_detail(complex_input_as_value_id: int, db: Session = Depends(get_db)):
    ...
    db_complex_input_as_value = crud.get_complex_input_as_value(
        db, complex_input_as_value_id=complex_input_as_value_id
    )
